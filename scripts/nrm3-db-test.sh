    ...
    if [ -z "${query}" ]; then
        ...
        runDbQuery "SELECT distinct REGEXP_REPLACE(recipe_name, '-.+', '') AS fmt FROM repository" 2>/dev/null | while read -r _fmt; do
            if [[ "${_fmt}" =~ \[fmt:([^\]]+)\] ]]; then
                ...
                echo "# repo_name, count, bytes for ${_format}"
                runDbQuery "${_sql}"
                local _sql="SELECT count(*) as count, SUM(ab.blob_size) as bytes FROM ${_format}_asset_blob ab LEFT JOIN ${_format}_asset a USING (asset_blob_id) WHERE a.asset_blob_id IS NULL"
                echo "# count, bytes for ${_format} (soft-deleting)"
                runDbQuery "${_sql}"
            fi
        done
    else
